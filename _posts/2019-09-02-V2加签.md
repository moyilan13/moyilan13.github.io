...
# 计算 apk 的摘要的源码解析
注释1-3处，computeContentDigests 函数返回的 "摘要算法ID - 分块摘要序列的摘要" 的 映射对，存储到变量 contentDigests 中。  
...
接着附上一张图示，来展示摘要计算的大致思路：
![](http://guoke24.top/img/计算APK的摘要.png)<br>
![](http://127.0.0.1:4000/img/计算APK的摘要.png)<br>
由上述代码段和图示，我们大致清楚了对一个 APK 摘要的双重计算的过程，得到的记过就是 **分块摘要序列的分块**。
